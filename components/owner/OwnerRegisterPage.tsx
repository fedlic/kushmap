...
export default function OwnerRegisterPage() {
  ...
  const handleClaim = async () => {
    ...
    if (error) {
      setError(error.includes('duplicate') ? 'This shop is already registered' : error)
    } else {
      setDone(true)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <span className="font-bold text-gray-900">Shop Owner Registration</span>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        {!user && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-700">
            Sign in to register your shop.
            <button onClick={() => setShowAuth(true)} className="underline font-medium mx-1">Sign in</button>
          </div>
        )}

        <div className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
          <h1 className="text-lg font-bold text-gray-900">Find Your Shop</h1>
          <p className="text-sm text-gray-500">Search by shop name and register as owner</p>
        ...
        {selected && !done && (
          <div className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
            <h2 className="font-bold text-gray-900">Selected Shop</h2>
            ...
            <div className="flex gap-3">
              ...
              >
                {claiming ? 'Registering...' : 'Register as Owner'}
              </button>
            </div>
          </div>
        )}
